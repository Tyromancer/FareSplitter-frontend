<template>
  <div>
    ...
    <b-form>
      ...
      <b-button @click="add_payment" variant="outline-primary"
        >Add another payment</b-button
      >

      <div v-for="(payment, counter) in this.payments" :key="counter">
        <b-button variant="outline-danger" @click="delete_payment(counter)"
          >x</b-button
        >
        <b-form-group
          :label="counter + ': Username'"
          :label-for="'username_' + counter"
        >
          <b-form-input
            :id="'username_' + counter"
